  ...
  sock.ev.on('group-participants.update', async (update) => {
    try {
      ...
      if (isWelcome) {
        ...
        ctx.fillText('¡Hasta luego', canvas.width / 2, 90)
        ctx.fillText('de', canvas.width / 2, 140)
        ctx.fillText(groupName, canvas.width / 2, 190)
      }

      ctx.font = '32px Sans'
      ctx.fillText(displayName, canvas.width / 2, isWelcome ? 190 : 260)
