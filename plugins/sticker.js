...
export default {
  ...
  execute: async ({ sock, m, args, pushName }) => {
    try {
      ...
      // Lógica de nombres: /s Pack | Autor O /s Pack
      ...
      if (args.length > 0) {
        ...
        if (str.includes('|')) {
          ...
          pack = str.trim() // Si solo hay una palabra, es el nombre del pack
        }
      }

      const stream = await downloadContentFromMessage(
        message[type],
        type === 'imageMessage' ? 'image' : type === 'videoMessage' ? 'video' : 'sticker'
      )
      ...
      if (type === 'stickerMessage') {
        ...
        if (type === 'imageMessage') {
          ...
          await runFfmpeg([
            ...
            '-vcodec', 'libwebp', '-loop', '0', '-t', '6', '-an', output
          ])
        }
      ...
      await sock.sendMessage(m.key.remoteJid, { sticker: finalBuffer }, { quoted: m })
